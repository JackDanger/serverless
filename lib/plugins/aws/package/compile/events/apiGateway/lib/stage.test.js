...
describe('#compileStage()', () => {
  ...
  beforeEach(() => {
    ...
    accountLogicalid = awsCompileApigEvents.provider.naming.getApiGatewayAccountLogicalId();
    logsRoleLogicalId = awsCompileApigEvents.provider.naming.getApiGatewayLogsRoleLogicalId();
  ...
  describe('logs', () => {
    ...
    describe('when the config contains the arn for a specific role', () => {
      beforeEach(() => {
        // setting up API Gateway logs
        awsCompileApigEvents.serverless.service.provider.logs = {
          restApi: {
            role: 'arn:aws:iam::1234567890:role/existing-secure-role',
          },
        };
      });
      it('should not create a new IAM Role resource', () =>
        awsCompileApigEvents.compileStage().then(() => {
          const resources =
            awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources;

          expect(resources[logsRoleLogicalId]).not.to.exist;
          expect(resources[accountLogicalid]).to.deep.equal({
            Type: 'AWS::ApiGateway::Account',
            Properties: {
              CloudWatchRoleArn: 'arn:aws:iam::1234567890:role/existing-secure-role',
            }});
        }))
    });
