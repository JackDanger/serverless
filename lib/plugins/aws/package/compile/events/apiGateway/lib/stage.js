...
module.exports = {
  compileStage() {
    ...
    const roleArn = logs && provider.logs.restApi.role;
    ...
    const logsRoleLogicalId = this.provider.naming.getApiGatewayLogsRoleLogicalId();
    const accountLogicalid = this.provider.naming.getApiGatewayAccountLogicalId();
    ...
    if (logs || TracingEnabled || Tags.length > 0) {
      ...
      if (logs) {
        ...
        if (roleArn) {
          // If the config contains an existing role ARN, use that
          _.merge(cfTemplate.Resources, {
            [logGroupLogicalId]: getLogGroupResource(service, stage),
            [accountLogicalid]: useLogRoleArn(roleArn),
          });
        } else {
...
function useLogRoleArn(roleArn) {
  return {
